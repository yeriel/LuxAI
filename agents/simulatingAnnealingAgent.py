...
def possible_ActionA(actions, player, observation, city_tiles, build_city, resource_tiles, cities, workers):
...
def possible_ActionB(actions, player, observation, city_tiles, build_city, resource_tiles, cities, workers):
    for unit in player.units:
        if unit.is_worker() and unit.can_act():
...
def acceptance(energy, e , Temp):
    if energy > e:
        return 1
    return math.exp(-(energy - e)/Temp)
